...
_Exchange_Daily_Hours = Dict(
    ...
    "Paris"=> Dict(:h =>8.5,:name =>"Euronext Paris"),
...
function AssetPrice(ticker::String,from::TimeType,to::TimeType,freq::String;exchange_local_time=true)
    p = get_prices(ticker,startdt = from, enddt = to , interval=freq,exchange_local_time=exchange_local_time,autoadjust=false)
...
function DataFrame(data::AssetPrice)
    ...
    return DataFrame(Date = dates, AdjClose = prices)
end

function DataFrame(data::AssetReturn)
    dates = Date.(data.timestamp)  # Convert DateTime to Date
    returns = round.(data.values, digits=6)  # Extract returns and round to 6 decimals
    return DataFrame(Date = dates, Return = returns)
end